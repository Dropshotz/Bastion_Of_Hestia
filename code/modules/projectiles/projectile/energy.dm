...
//boh
/obj/item/projectile/energy/enoverstrong
	name = "bolt"
	taser_effect = 1
	damage = 2
	damage_type = BURN
...
//The below, seeing as it's copied from 'plasmastun', likely won't work. It'd be interesting if it does, however. -Carl

/obj/item/projectile/energy/enoverstrong/proc/bang(var/mob/living/carbon/M)

	to_chat(M, "<span class='danger'>You hear something akin to an explosion!</span>")
	playsound(M.loc, 'sound/effects/bang.ogg', 50, 1)
	var/ear_safety = 0
	ear_safety = M.get_ear_protection()
	if(ear_safety == 1)
		...
		M.ear_deaf = max(M.ear_deaf,15)
	if (M.ear_damage >= 15)
		to_chat(M, "<span class='danger'>Your ears ring as the sense of vertigo overwhelms you!</span>")
		if (prob(M.ear_damage - 5))
			to_chat(M, "<span class='danger'>You can't hear anything!</span>")
			M.sdisabilities |= DEAF
	else
...
/obj/item/projectile/energy/enoverstrong/on_hit(var/atom/target)
	bang(target)
	. = ..()
